...
impl<'a> CartesianMotionPlanner<'a> {
    ...
    fn get_current_velocity(&self) -> Point3 {
        Point3::new(
            self.x_motor.get_velocity(),
            self.y_motor.get_velocity(),
            self.z_motor.get_velocity(),
        )
    }
}
...
impl<'a> MotionPlanner for CartesianMotionPlanner<'a> {
    fn set_target(&mut self, point: Point3) {
        ...
        self.top_x_speed = self.translate_to_axis(
            self.delta_x,
            self.get_axis_top_speed(self.delta_x, self.max_speed),
        );
        self.top_y_speed = self.translate_to_axis(
            self.delta_y,
            self.get_axis_top_speed(self.delta_y, self.max_speed),
        );
        self.top_z_speed = self.translate_to_axis(
            self.delta_z,
            self.get_axis_top_speed(self.delta_z, self.max_speed),
        );
        ...
        let part1_distance = self.start_speed * self.transition1 as f32 +
            (1.0 / 2.0) * self.max_acceleration * (self.transition1 * self.transition1) as f32;
    ...
    fn update(&mut self) {
        ...
        let now = self.time.now() - self.start_time;
        ...
        self.x_motor
            .set_velocity((current_speed * self.delta_x) / self.motion_distance);
        self.y_motor
            .set_velocity((current_speed * self.delta_y) / self.motion_distance);
        self.z_motor
            .set_velocity((current_speed * self.delta_z) / self.motion_distance);
    }
}
